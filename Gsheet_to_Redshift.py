...
dag = DAG(
    ...
    start_date = datetime(2023,2,1), # 날짜가 미래인 경우 실행이 안됨
...
sheets = [
    {
        "url": "https://docs.google.com/spreadsheets/d/1SQPQYRhoY9mTXcC7G8cuoRKwSehb-8mmqFJ1lXc8bpg/edit?usp=sharing",
        "tab": "demo",
        "schema": "wkdansqls",
...
for sheet in sheets:
    ...
    run_copy_sql = S3ToRedshiftOperator(
        ...
        s3_bucket = "grepp-data-engineering",
        s3_key = sheet['schema'] + '-' + sheet["table"],
        schema = sheet["schema"],
        table = sheet["table"],
